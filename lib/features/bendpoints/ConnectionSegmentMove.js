...
function ConnectionSegmentMove(injector, eventBus, canvas, dragging, graphicsFactory, rules, modeling) {
  ...
  eventBus.on('connectionSegment.move.move', function(e) {
    ...
    if (segmentStartIndex < 2) {
      ...
      var source = { x: connection.source.x, y: connection.source.y };
    ...
    newSegmentStartIndex += segmentOffset;

    // handle segments between first and last waypoint
    if (segmentStartIndex >= 2) {

      var segmentBeforeStart = context.originalWaypoints[segmentStartIndex-1];
      sourceToSegmentOrientation = LayoutUtil.getOrientation(segmentBeforeStart, newSegmentStart, 1);

      if (sourceToSegmentOrientation === 'intersect') {
        newSegmentStartIndex -= 2;
      }
    }
    ...
    context.newSegmentStartIndex = newSegmentStartIndex;
